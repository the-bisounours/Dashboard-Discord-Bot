...
module.exports = {
    ...
    execute: async (client, interaction) => {
        ...
        interaction.values.map(value => {
            const index = voice.trust.indexOf(value);
            if (index > -1) {
                voice.trust.splice(index, 1);
            };
        });
        ...
        return await interaction.update({
            content: `${client.emo.yes} ${interaction.values.length === 1 ? "L'utilisateur" : "Les nouveaux utilisateurs"} ${interaction.values.map(value => `\`${interaction.guild.members.cache.get(value).displayName}\``).join(", ")} ${interaction.values.length === 1 ? "n'est" : "ne sont"} plus de confiance.`,
            components: [],
            ephemeral: true
        });
    }
