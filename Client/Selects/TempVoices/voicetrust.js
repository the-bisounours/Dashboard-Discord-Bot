...
module.exports = {
    ...
    execute: async (client, interaction) => {
        ...
        interaction.values.map(value => {
            if(!voice.trust.includes(value)) {
                voice.trust.push(value);
            };
        });
        ...
        return await interaction.update({
            content: `${client.emo.yes} ${interaction.values.length === 1 ? "L'utilisateur" : "Les nouveaux utilisateurs"} ${interaction.values.map(value => `\`${interaction.guild.members.cache.get(value).displayName}\``).join(", ")} ${interaction.values.length === 1 ? "est de confiance" : "sont de confiance"}.`,
            components: [],
            ephemeral: true
        });
    }
