...
module.exports = {
    ...
    execute: async (client, interaction) => {
        ...
        panel.buttons[index].label = interaction.fields.getTextInputValue("label");
        panel.buttons[index].customId = `ticket-${interaction.fields.getTextInputValue("label")}`;
        await data.updateOne({
            [`tickets.panels.${data.tickets.panels.findIndex(panel => panel.panelId === interaction.customId.split("_")[3])}.buttons.${index}.label`]: interaction.fields.getTextInputValue("label"),
            [`tickets.panels.${data.tickets.panels.findIndex(panel => panel.panelId === interaction.customId.split("_")[3])}.buttons.${index}.customId`]: `ticket-${interaction.fields.getTextInputValue("label")}`,
        });
